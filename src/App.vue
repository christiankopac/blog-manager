...
import { computed, onMounted, ref, onUnmounted, CSSProperties, watch, nextTick } from "vue";
...
// Modal management
const { showModal, selectedPost, openCreateModal, openEditModal, closeModal } =
  usePostModal();
...
const allPostsSelected = computed(
  () => selectedPosts.value.length === filteredPosts.value.length
);
...
const toggleSelectAll = () => {
  selectedPosts.value = allPostsSelected.value
    ? []
    : filteredPosts.value.map((post) => post.id);
...
const ITEM_HEIGHT = 58; // Height of each post row in pixels
...
const { 
  ...
  handleScroll,
  forceUpdate 
} = useVirtualScroll(filteredPosts, {
...
<template>
  ...
  <div class="min-h-screen h-screen flex flex-col bg-gray-100 dark:bg-gray-900">
    <header
      class="shrink-0 px-4 sm:px-6 py-4 bg-gray-800 dark:bg-gray-800 border-b dark:border-gray-700"
    >
    ...
    <main role="main" class="flex-1 flex flex-col px-2 sm:px-6 py-4 min-h-0">
      <div class="max-w-screen-xl mx-auto w-full h-full flex flex-col">
        <div
          class="shrink-0 flex flex-col sm:flex-row gap-2 sm:gap-4 mb-4 w-full"
        >
          <label for="search-posts" class="sr-only">Search posts</label>
          <div class="flex-1">
            <SearchBar v-model="searchQuery" :is-loading="store.isLoading" />
          </div>
        ...
        <div
          v-if="store.error && !store.isLoading && filteredPosts.length > 0"
          role="alert"
          class="relative px-4 py-3 bg-red-100 dark:bg-red-900/20 border-b border-red-200 dark:border-red-900/30 text-red-600 dark:text-red-400"
        >
          {{ store.error }}
          <button
            @click="store.error = null"
            class="absolute top-2 right-2 text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300"
          >
        ...
        <div
          class="flex-1 flex flex-col min-h-0 border border-gray-200 dark:border-gray-700 overflow-hidden"
        >
          <TableHeader
            :all-posts-selected="allPostsSelected"
          ...
          >
            ...
            >
              <div
                v-for="n in 5"
                :key="n"
                class="grid grid-cols-11 animate-pulse sm:grid-cols-11"
              >
            ...
            >
              <div
                v-for="{ data: post, index, style } in visibleItems"
                :key="post.id"
              ...
              >
                <PostRow
                  ...
                  :user-name="
                    store.users.find((user) => user.id === post.userId)?.name ||
                    ''
                  "
